...
async function sendMessage() {
  ...
  try {
    // [CORS 해결] 가장 간편한 공개 프록시 사용
    const proxyUrl = 'https://corsproxy.io/?';
    const targetUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GOOGLE_API_KEY}`;

    // 3) Gemini API 호출 (프록시 주소와 타겟 주소를 합침)
    ...
    if(data.candidates && data.candidates[0].content) {
      ...
      const errorMsg = data.error ? data.error.message : '알 수 없는 에러가 발생했습니다.';
      appendMessage('bot', `오류: ${errorMsg}`);
      console.log('Error Data:', data);
    }

  } catch(error) {
    console.log('Fetch error:', error);
    appendMessage('bot', '네트워크 오류가 발생했습니다. (프록시 서버 확인)');
  } finally {
    // 5) 로딩 끄기
    showLoading(false);
  }

...
/**
 * 실무 사용 사례
 * 전화번호 입력 받을 때 - 없어야 하면 걸러내야하기 때문에 '010-1234-5678'이라는 string 값(number라면 010이 불가능합니다)에 .indexOf('-')를 확인하면 -1이 return되면 다음 단계로, 3번지가 출력된다면 '-'을 제거하는 메서드로 넘길 수 있습니다.
 */
...
console.log(banana);  // slice(시작점, 한계값); 입니다. 한계값 미만까지 출력됩니다.
let slicedFruits = strFruits.slice(7);  // 이러면 7번지부터 끝까지
console.log(slicedFruits);  // 결과값 : Banana, Kiwi
// python에서는 마이너스 인덱스가 있기는 한데, JS에서는 없습니다. 하지만 slice()의 argument로 마이너스값을 넣어줄 수는 있습니다.
...
// SEOUL은 찾고자하는 문자열 i는 insensitive의 약자로 덜까다롭게 굴겠다는 의미입니다.
...
// toUpperCase() / toLowerCase() 대 / 소문자 바꿔주는거
...
/**
 * 즉 .split()의 결과값의 자료형은 Array에 해당됩니다. 즉 argument로 넣은 '-'를 만날 때 마다 배열의 element로 집어넣는다고 볼 수 있습니다.
